...
{ 
    ...
    // Initialize remaining_time[] as a copy of plist[].bt
    for (int i = 0; i < n; i++)
    ...
    while (completed_processes < n)
    {
        ...
        // Traverse all processes to find the one with the minimum remaining time
        for (int i = 0; i < n; i++)
        {
        ...
        {
            completed_processes++;
            plist[min_index].tat = current_time + 1;
            plist[min_index].wt = plist[min_index].tat - plist[min_index].art - plist[min_index].bt;
        }
